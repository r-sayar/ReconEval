...
class ReconPretrainedStateModel(BaseReconstructionModel):
    def __init__(
        ...
        **kwargs
        ...
        """Pre-trained State Embedding (SE) foundation model :cite:`adduri:25`.

        Wraps the SE encoder for ReconEval's foundation-model
        reconstruction task. Requires the ``state`` package
        and runs in the ``cstm_scvi_env`` conda env.
    ...
    def train(self, datamodule: L.LightningDataModule = None, **train_kwargs):
        """No-op: SE is used frozen. Raises if a datamodule is passed."""
        if datamodule is not None:
            raise NotImplementedError(
                "Fine-tuning is not supported for the pretrained SE wrapper."
            )
